...
export const useZoomController = (options: UseZoomControllerOptions = {}) => {
  ...
  const applyTargetScroll = useCallback(
    (containerRef: React.RefObject<HTMLDivElement | null>, onScrollUpdate: (x: number, y: number) => void) => {
      const container = containerRef.current;
      if (!container) return;
      ...
      onScrollUpdate(x != null ? x : container.scrollLeft, y != null ? y : container.scrollTop);
