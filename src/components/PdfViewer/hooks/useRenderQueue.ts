...
export function useRenderQueue(updatePageRender: (pageIndex: number, pageLayout: PageLayoutLike) => Promise<void>) {
  ...
  const enqueue = useCallback(
    (items: EnqueueTask | EnqueueTask[]) => {
      const list = Array.isArray(items) ? items : [items];
      const epoch = currentEpochRef.current;

      for (const it of list) {
        // 去重：若已有相同页面的待处理任务，保留优先级更高的那个
        const existingIdx = queueRef.current.findIndex((t) => t.epoch === epoch && t.pageIndex === it.pageIndex);
        if (existingIdx >= 0) {
